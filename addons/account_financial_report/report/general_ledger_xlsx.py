    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        ...
        for account in general_ledger:
            ...
            if not account["partners"]:
                ...
                account.update({
                    "final_debit": max(account["fin_bal"]["debit"], 0),
                    "final_credit": max(account["fin_bal"]["credit"], 0),
                    "final_balance": max(account["fin_bal"]["balance"], 0),
                })

                if foreign_currency:
                    account.update(
                        {
                            "final_bal_curr":max(account["fin_bal"]["bal_curr"], 0),
                        }
                    )
                ...
                for partner in account["list_partner"]:
                    ...
                    if foreign_currency and partner["currency_id"]:
                        partner.update(
                            {
                                "final_bal_curr":max(partner["fin_bal"]["bal_curr"], 0),
