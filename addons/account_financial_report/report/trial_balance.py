...
class TrialBalanceReport(models.AbstractModel):
    ...
    def _compute_account_amount(self, total_amount, tb_initial_acc, tb_period_acc, foreign_currency):
        for tb in tb_period_acc:
            ...
            total_amount[acc_id] = {}
            total_amount[acc_id]["credit"] = max(tb["credit"], 0)
            total_amount[acc_id]["debit"] = max(tb["debit"], 0)
            total_amount[acc_id]["balance"] = max(tb["balance"], 0)
            total_amount[acc_id]["initial_balance"] = 0.0
            total_amount[acc_id]["ending_balance"] = max(tb["balance"], 0)
            if foreign_currency:
                total_amount[acc_id]["initial_currency_balance"] = 0.0
                total_amount[acc_id]["ending_currency_balance"] = max(round(tb["amount_currency"], 2), 0)
        ...
        for tb in tb_initial_acc:
            ...
            if acc_id not in total_amount.keys():
                ...
                total_amount[acc_id]["initial_balance"] = max(tb["balance"], 0)
                total_amount[acc_id]["ending_balance"] = max(tb["balance"], 0)
                if foreign_currency:
                    total_amount[acc_id]["initial_currency_balance"] = max(round(tb["amount_currency"], 2), 0)
                    total_amount[acc_id]["ending_currency_balance"] = max(round(tb["amount_currency"], 2), 0)
            else:
                total_amount[acc_id]["initial_balance"] = max(tb["balance"], 0)
                total_amount[acc_id]["ending_balance"] += max(tb["balance"], 0)
    ...
    def _compute_acc_prt_amount(self, total_amount, tb, acc_id, prt_id, foreign_currency):
        ...
        total_amount[acc_id][prt_id]["credit"] = max(tb["credit"], 0)
        total_amount[acc_id][prt_id]["debit"] = max(tb["debit"], 0)
        total_amount[acc_id][prt_id]["balance"] = max(tb["balance"], 0)
        total_amount[acc_id][prt_id]["initial_balance"] = max(tb["balance"], 0)
        total_amount[acc_id][prt_id]["ending_balance"] = max(tb["balance"], 0)
        if foreign_currency:
            total_amount[acc_id][prt_id]["initial_currency_balance"] = max(round(tb["amount_currency"], 2), 0)
            total_amount[acc_id][prt_id]["ending_currency_balance"] = max(round(tb["amount_currency"], 2), 0)
        return total_amount
    ...
    def _get_report_values(self, docids, data):
        ...
        if not show_partner_details:
            for account_id in accounts_data.keys():
                ...
                {
                    "initial_balance": max(total_amount[account_id]["initial_balance"], 0),
                    "credit": max(total_amount[account_id]["credit"], 0),
                    "debit": max(total_amount[account_id]["debit"], 0),
                    "balance": max(total_amount[account_id]["balance"], 0),
                    "ending_balance": max(total_amount[account_id]["ending_balance"], 0),
                    "type": "account_type",
                }
